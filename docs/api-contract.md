...
- `POST /pos/device-login` — `{code, device_secret}` → `{token, abilities, store_id}`. For API-integrated customers whose physical POS terminal calls this API directly, with no human operator login step. `code` + `device_secret` are the terminal's own device-level credentials, issued once (and rotatable) from the admin panel — never a User's email/password. Issues a Sanctum token scoped to the terminal's own store: same abilities as `/pos/login` plus a synthetic `pos:{hashid}` ability instead of a human identity. Shifts/transactions opened with this token carry a filled `pos_id` and leave `operator_id` null — since the terminal itself, not whichever person is standing at it, is the fixed identity worth tracking. `422` on an unknown `code` or wrong `device_secret` (same convention as `/pos/login`).
...
### Shift
- `POST /shifts` (open) — `{store_id}`. Requires the caller's token to carry a `store:{hashid}` ability matching this store (`403` otherwise). A device-login token (POS terminal) records `pos_id` and leaves `operator_id` null; an operator-login token (User) does the opposite — see Auth (POS).
